...
def get_pv_system(panel):
    ...
    if inverter_pdc0 > 200:
        ...
        eta_inv_nom = 0.96
    inverter_parameters = {"pdc0": inverter_pdc0, "eta_inv_nom": eta_inv_nom}
...
def simulate_pv_output(system, weather_data, location):
    ...
    for array in system.arrays:
        ...
        aoi = irradiance.aoi(
            array.mount.surface_tilt,
            array.mount.surface_azimuth,
        ...
        dc_output = single_array_system.pvwatts_dc(
            poa_irrad["poa_global"], cell_temperature
        )
...
def fit_gaussian_to_daily_data(daily_data):
    ...
    popt, _ = curve_fit(
        gaussian, x_numeric, daily_data, p0=[max(daily_data), np.argmax(daily_data), 1]
    )
    return popt
    ...
    for i in range(num_panels):
        ...
        for day in range(days_to_plot):
            ...
            if popt is not None:
                ...
                x_dense = np.linspace(0, 23, 500)
                x_plot = pd.date_range(
                    start=energy_outputs.index[start_idx], periods=24, freq="H"
                )
                x_plot_dense = pd.date_range(
                    start=x_plot[0], end=x_plot[-1], periods=500
                )
                plt.plot(
                    x_plot_dense,
                    gaussian(x_dense, *popt),
                    color=colors2[i],
                    label=f"Gaussian Panel {i+1}, {area_difference:.2f} Wh",
                    linewidth=2,
                )

            # Plot the original daily mean energy data
            plt.plot(
                energy_outputs.index[start_idx:end_idx],
                daily_data,
                label=f"Panel {i+1} Mean",
                linestyle="--",
                color=colors[i],
            )
            ...
            end_time = energy_outputs.index[
                min(len(energy_outputs.index), days_to_plot * 24) - 1
            ]
            plt.plot(
                data[start_time:end_time].index,
                data[start_time:end_time]["W.mean_value"],
                color="black",
                label="W.mean_value",
                linewidth=2,
                linestyle="-.",
...
if __name__ == "__main__":
    ...
    weather_data = load_weather_data(
        "energy_prediction/energy_data/historical_weather.csv"
    )
    ...
    site_location = location.Location(
        latitude=52.52, longitude=13.4050, altitude=34, tz="Europe/Amsterdam"
    )
    ...
    energy_outputs.to_csv(
        "energy_prediction/energy_data/energy_output.csv", sep=",", index=True
    )
